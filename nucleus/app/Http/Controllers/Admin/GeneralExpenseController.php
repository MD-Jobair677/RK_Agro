...
class GeneralExpenseController extends Controller
{
    ...
    public function index()
    {
        ...
        $pageTitle = 'General Expense History';
        $genExpenses = GeneralExpense::searchable(['name'])->dateFilter()->orderBy('id', 'desc')->latest()->paginate(getPaginate());
        return view('admin.accounts.genexpense_index', compact('pageTitle', 'genExpenses'));
    }
    ...
    {
        return Cattle::whereIn('status', [1, 2])
            ->where('cattle_category_id', $categoryId)
            ->get()
            ->filter(function ($cattle) {
                if ($cattle->type == ManageStatus::PURCHASE_CATTLE) return true;

                if ($cattle->type == ManageStatus::BORN_CATTLE) {
                    return Carbon::parse($cattle->purchase_date)->addYear()->lte(now());
                }
